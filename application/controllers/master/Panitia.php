...
class Panitia extends AUTH_Controller {
	...
	public function index() {
		...
		$data['panitia'] = $this->GeneralApiModel->getWhereTransactionalOrdered(array('1'=>1),'namalengkap', 'ASC', 'user_panitia_detail')->result();

		$this->template->views('admin/master-panitia', $data);
	}

	public function byrole() {
		$data['page'] = "panitia";
		$role = array('','','Administrator', 'Operator', 'Pemateri');
		$data['title'] = "Daftar ".$role[$_GET['role']];
		$data['panitia'] = $this->GeneralApiModel->getWhereTransactionalOrdered(array('role'=>$_GET['role']),'namalengkap', 'ASC', 'user_panitia_detail')->result();
